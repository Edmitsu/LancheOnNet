      ...
      <div class="popup-content">
        ...
        <div class="content">
          ...
          <div class="details-container">
            ...
            <p><strong>Total: R$ {{ (selectedCombo.price * quantidade).toFixed(2) }}</strong></p>
          </div>
        </div>
        <div class="cta">
          <button @click="adicionarPedido">Fazer Pedido</button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async adicionarPedido() {
      ...
      const { name, description, price } = this.selectedCombo;

      const pedido = {
        nome: name,
        descricaoCombo: description,
        qtdCombo: this.quantidade,
        preco: price * this.quantidade,
        quantidade: this.quantidade
      };

      try {
        // Adicione o pedido aos pedidos
        const responsePedido = await axios.post('http://localhost:3000/pedidos/', pedido);
        const numeroPedido = responsePedido.data.pedido.numeroPedido;

        // Adicione o pedido ao carrinho
        const responseCarrinho = await axios.post('http://localhost:3000/carrinho/', {
          pedidoId: responsePedido.data.pedido._id
        });

        if (responseCarrinho.status === 200 && responsePedido.status === 200) {
          ...
          console.error('Erro ao adicionar pedido ao carrinho.');
        }
      } catch (error) {
...
export default {
  ...
  methods: {
    ...
    getTotalPrice() {
      if (this.selectedCombo) {
        return this.selectedCombo.price * this.quantidade;
      }
      return 0;
    },
    async adicionarPedido() {
